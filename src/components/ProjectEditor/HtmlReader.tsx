...
export function HtmlReader({
  ...
  const applyPendingScrollRestore = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && stateRef.current?.browserId === current.browserId) {
  ...
  useEffect(() => {
    ...
    if (restoreScrollState) window.setTimeout(() => void applyPendingScrollRestore(), 0)
  }, [applyPendingScrollRestore, restoreScrollState])
