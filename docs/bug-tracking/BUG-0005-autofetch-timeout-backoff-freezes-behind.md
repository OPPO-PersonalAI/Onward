...
| R3 | `↓M` is de-emphasised (muted, normal weight) once the last SUCCESSFUL fetch is older than 20 min — or never happened despite attempts. When `behind === 0` and stale it renders `↓?` instead of vanishing, because "no arrow" reads as "you are up to date", which is the exact false statement that produced this report. Tooltip gains a second line | `gitSyncDisplay.ts::resolveGitSyncFreshness` (pure), `TerminalGrid.tsx`, `TerminalGrid.css`, `src/i18n/core.ts` (en + zh-CN) | Fed by `lastFetchOkAt` / `lastFetchAttemptAt`, stamped onto the snapshot by the router and carried on the EXISTING mirror-delta channel — no new IPC surface |
