...
| 2026-07-23 (eve) | **Escalation + the strongest repro evidence yet (bundle `onward-diagnostic-2026-07-23_19-24-53.zip`, machine 1 prod 07.22).** User reports the crash now reproduces at HIGH probability on every invisible→visible cycle. Trace: hidden 19:24:10.709 (keepalive, 6 WebGL sessions) → parked only ~18 s → visible+focus 19:24:28.356 → `gpu.process-gone {crashed, exitCode:5}` at 19:24:28.360 — **4 ms after wake, no watchdog involvement** (last watchdog activity 3 h prior). Counter-evidence in the same trace: a 19:14–19:15 flurry of blur/focus WITHOUT visibility-change produced zero crashes → the trigger discriminator is the document hidden→visible transition (real occlusion), not focus. Differential vs the same-day zero-crash 150-cycle stress (same machine): park duration 18 s vs 90 ms, GPU-process age (hours serving 6 busy terminals vs fresh instance), real occlusion vs `win.hide()`, dual 5K displays. Repro recipe for the soak harness: real hidden→visible cycles, 15–30 s parks, live PTY output during park, aged/pre-warmed GPU process. Antecedent set now: wake +30 ms (m2), throttle-toggle +14 ms (m1), wake +4 ms (m1 eve). |
