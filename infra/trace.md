...
| Constant | Name | Phase | Call site |
|---|---|---|---|
| `MAIN_FILE_INDEX_BUILD` | `main:file-index.build` | `X` (has `durationMs`) | `electron/main/ipc-handlers.ts` `PROJECT_BUILD_FILE_INDEX` handler. Payload also carries `fileCount` and `strategy` (`ripgrep` / `walk-fallback`). `walk-fallback` means the bundled ripgrep could not be run, so `.gitignore` was NOT applied and the index fell back to the coarse hard-coded ignore list — the first thing to check when a report says Cmd+P is full of build artefacts. |
| `MAIN_FILE_INDEX_UPDATE` | `main:file-index.update` | `i` | Same, `PROJECT_INVALIDATE_FILE_INDEX` handler (`reason: 'invalidate'`) and `PROJECT_PATCH_FILE_INDEX` handler (`reason: 'patch'`, plus `added` / `removed` / `renamed` counts and `applied` / `changed` / `fileCount`). A trace dominated by `'invalidate'` where the diff was knowable means a call site regressed back to blanket invalidation, which costs a full recursive re-walk on the next search. |
| `RENDERER_FILE_INDEX_MIRROR_SYNC` | `renderer:file-index.mirror-sync` | `i` | `src/components/ProjectEditor/GlobalSearch/fileIndexCacheBootstrap.ts` watcher handler. Payload: `reason` (`patch-applied` / `patch-noop` / `resync-invalidate` / `patch-failed`), `added`, `removed`, and `fileCount` (the authority's post-patch count, which the renderer mirrors rather than computing). `patch-noop` is the healthy signature of a plain file save — the watcher reported the path but the file set did not move. |
| `RENDERER_FILE_INDEX_SEARCH_PAGE` | `renderer:file-index.search-page` | `i` | `src/components/ProjectEditor/GlobalSearch/useFilenameSearch.ts`, page 0 fetch and each `loadMore`. Payload: `queryLen`, `offset`, `limit`, `returned`, `total`, `hasMore`. Makes "why did the list stop here" answerable from a user-supplied trace. |
