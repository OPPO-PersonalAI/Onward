...
| `MAIN_GIT_DIFF_WARM_STATUS_REUSE` | `main:git.diff.warm-status-reuse` | `i` | `electron/main/git-utils.ts::getSingleRepoDiff` (hit/stale) + `git-ipc-worker-entry.ts::warmDiffCache` (unavailable) — whether a BACKGROUND diff warm reused the mirror's presupplied parent status instead of spawning its own (G2 C-i, warm path only; 15 s age gate). Foreground opens never take this path. Tagged `cwd`, `result: 'hit' \| 'stale' \| 'unavailable'`, `ageMs?`, `fileCount?`. |
