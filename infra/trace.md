...
| `MAIN_TERMINAL_CHANGE_WORKDIR_FAILED` | `main:terminal.change-workdir-failed` | `i` | Same file — transaction failed (`target-not-found` / `terminal-not-found` / `write-failed` / `verify-timeout`); nothing was persisted. Tagged `terminalId`, `reason`, `path?`, `waitedMs?`. |
