...
| `MAIN_VISIBILITY_WATCHDOG_CHECK_VERDICT` | `main:visibility-watchdog.check-verdict` | `i` | `visibility-watchdog.ts::runCheck` — every check whose verdict is `mismatch` OR whose prior status ≠ ok (BUG-0002: the silent cooldown observation stretch was invisible). Routine background not-applicable stream stays silent. Tagged `reason`, `verdict`, `windowVisible`, `windowFocused`, `probe`, `consecutiveMismatches`, `status`. |
| `MAIN_VISIBILITY_WATCHDOG_NUDGE_OUTCOME` | `main:visibility-watchdog.nudge-outcome` | `i` | Same — post-nudge recheck pairing a nudge with its direct effect. Tagged `level`, `verdictAfter`, `status`. |
| `MAIN_VISIBILITY_WATCHDOG_INPUT_REPORT` | `main:visibility-watchdog.input-report` | `i` | `visibility-watchdog.ts` IPC handler — preload reported user input on a hidden document (throttled 1/5 s preload-side); triggers an immediate cooldown-bypassing check. Tagged `hasFocus`. |
| `MAIN_WINDOW_LIFECYCLE` | `main:window.lifecycle` | `i` | `visibility-watchdog.ts::recordWindowLifecycle` — main-side BrowserWindow show/hide/focus/blur/minimize/restore transitions with nudge attribution (BUG-0002 open question #1: user-vs-nudge focus at nudge time was indistinguishable). Tagged `event`, `isVisible`, `isFocused`, `isMinimized`, `trigger: 'user' \| 'watchdog-nudge'`. |
| `MAIN_DEBUG_GPU_PROCESS_KILL` | `main:debug.gpu-process-kill` | `i` | `ipc-handlers.ts` DEBUG_KILL_GPU_PROCESS (autotest-gated) — a REAL SIGKILL was sent to the GPU helper pid; the ensuing `main:gpu.process-gone` carries reason 'killed', distinguishing test-induced kills from organic crashes. Tagged `pid`, `found`, `gpuEntryCount`. |
| `MAIN_GPU_SOAK_CYCLE` | `main:gpu-soak.cycle-start` | `i` | `ipc-handlers.ts` DEBUG_RUN_GPU_WAKE_PARK_SOAK — one wake-park soak cycle began (hide → park with live PTY output → show+focus). Tagged `cycle`, `parkMs`, `throttleFlip`. Autotest-only. |
| `MAIN_GPU_SOAK_THROTTLE_FLIP` | `main:gpu-soak.throttle-flip` | `i` | Same — the mid-park backgroundThrottling off/on flip (the machine-1 crash antecedent) fired. Tagged `cycle`. |
| `MAIN_GPU_SOAK_CRASH_OBSERVED` | `main:gpu-soak.crash-observed` | `i` | Same — a GPU child-process-gone landed during a soak cycle (the reproducer scored a hit). Tagged `cycle`, `parkMs`. |
| `MAIN_UPDATER_STATE_CHANGED` | `main:updater.state-changed` | `i` | `update-service.ts::setStatus` — updater phase transitions (hourly-scale; BUG-0002/0003 "after upgrade" correlation had no trace anchor). Tagged `fromPhase`, `toPhase`, `targetVersion`. |
...
| `MAIN_GIT_STATE_MIRROR_CWD_SWITCHED` | `main:git-state-mirror.cwd-switched` | `i` | `electron/main/git-state-mirror-router.ts` — main forwards the cwd push from renderer to the worker. Tagged `terminalId`, `prevCwd`, `nextCwd`, `source` (`osc-renderer` / `osc-main` / `main-probe` / `unknown` — 2026-07 bundle fix: distinguishes shell-derived proof from attach-time probes). |
...
| `MAIN_TERMINAL_SHELL_INTEGRATION_SILENT` | `main:terminal.shell-integration-silent` | `i` | `electron/main/ipc-handlers.ts` (liveness callbacks; state machine in `electron/main/shell-integration-liveness.ts`) — no shell-derived cwd OSC arrived within the 15 s liveness window after a default-shell spawn (blocked integration / unsupported shell). Once per terminal lifetime. Tagged `terminalId`, `shellKind`, `waitedMs`. THE missing signal from the 2026-07-17 bundles: separates "integration blocked by policy" from "user never cd'd". |
| `MAIN_TERMINAL_SHELL_INTEGRATION_RECOVERED` | `main:terminal.shell-integration-recovered` | `i` | Same wiring — a shell-derived OSC arrived AFTER the silent verdict (policy fixed mid-session / very slow first prompt). Once per terminal. Tagged `terminalId`, `sinceSpawnMs`. |
| `MAIN_TERMINAL_CHANGE_WORKDIR_REQUESTED` | `main:terminal.change-workdir-requested` | `i` | `electron/main/terminal-change-workdir.ts` — verified manual cwd-switch transaction entered (RC-3 fix). User-click frequency. Tagged `terminalId`, `path`. |
| `MAIN_TERMINAL_CHANGE_WORKDIR_WRITTEN` | `main:terminal.change-workdir-written` | `i` | Same file — the cd + proof-OSC command was written into the PTY. Tagged `terminalId`, `shellKind`, `path`. |
| `MAIN_TERMINAL_CHANGE_WORKDIR_VERIFIED` | `main:terminal.change-workdir-verified` | `i` | Same file — the shell's own cwd report confirmed the switch; only now may the renderer persist. Tagged `terminalId`, `path`. |
| `MAIN_TERMINAL_CHANGE_WORKDIR_FAILED` | `main:terminal.change-workdir-failed` | `i` | Same file + the IPC gate in `ipc-handlers.ts` — transaction failed (`target-not-found` / `terminal-not-found` / `write-failed` / `verify-timeout` / `tui-active` — 2026-07-24 G1 gate: a full-screen TUI holds the alternate screen, the cd would be typed INTO it / `busy` — 2026-07-24 G2 lock: another transaction already in flight for this terminal); nothing was persisted. Tagged `terminalId`, `reason`, `path?`, `waitedMs?`. |
| `MAIN_GIT_REPO_META_PROBE_RESULT` | `main:git.repo-meta.probe-result` | `i` | `electron/main/git-utils.ts` `getGitRepoMeta` — outcome of an uncached rev-parse probe: `state` = `ok` / `not-repo` / `timeout` / `error` (RC-2 fix: `timeout` = killed at the exec budget on a hanging volume — previously misfiled as "not a repo"). Tagged `cwd`, `state`, `durationMs`, `timeoutStrikes`. Probe frequency is already dedupe/cache-bounded. |
...
| `RENDERER_GIT_STATE_MIRROR_SUBSCRIBE_ALIAS_REGISTERED` | `renderer:git-state-mirror.subscribe-alias-registered` | `i` | `src/components/TerminalGrid/TerminalGrid.tsx` — a raw→canonical cwd alias was registered from a `subscribeMirror` result. `cold:true` is the load-bearing case: a symlink/junction cwd whose repo was never warm used to skip alias creation entirely, leaving the Task badge empty (2026-07-16 symlink-status fix). Tagged `rawCwd`, `canonicalCwd`, `cold`. Subscribe-frequency (rare). |
| `RENDERER_GIT_STATE_MIRROR_UPDATE_RECEIVED` | `renderer:git-state-mirror.update-received` | `i` | `src/components/TerminalGrid/TerminalGrid.tsx` (mirror update listener) — **BUG-0005 P1**, DIAGNOSTIC tier (default-on, no `ONWARD_PERF_TRACE` needed). Main records `main:git-state-mirror.fanout` on send; nothing recorded the RECEIVE at any tier, so "main computed it, the badge never moved" had no evidence either way. Tagged `cwd`, `deltaKeyCount`, `hasAheadBehind`, `hasFetchFreshness`. Rate control: throttled to one per cwd per 250 ms — a `git pull` checking out thousands of files produces a fanout burst (a field bundle dropped 7,903 watcher events in one window) and the diagnostic must observe it, not amplify it. |
...
| `WORKER_GIT_STATE_MIRROR_RECOMPUTE_DONE` | `worker:git-state-mirror.recompute-status-done` | `X` (duration) | `git-state-mirror-worker-entry.ts` — wraps a single `git status --porcelain=v2 -z` run plus delta computation. Payload: `cwd`, `repoRoot`, `reason` (`attach` / `watcher` / `polling` / `osc-switch` / `focus-resync` / `reconcile`), `fileCount`, `branch`, `status`, `ahead`/`behind`/`hasUpstream` (**BUG-0005 P0** — the VALUES, not just the fact that they changed: `fanout` records `deltaKeys`, so a stale-badge trace could prove "behind changed" but never "behind changed from 3 to 0", which is the only form that settles a "the number is wrong" report), `durationMs` (**BUG-0005 R5** — sanity-checked via `sanitizeMeasuredDurationMs`; a sample spanning a system sleep is reported as 0 rather than being fed to the adaptive backoff), `repoProbe` (`ok` / `not-repo` / `timeout` / `error` — RC-2 classification; timeout probes also enter the worker's 30 s → 2 min → 5 min backoff, surfaced as `recompute-deferred` with reason `probe-backoff`). |
...
| `WORKER_GIT_STATE_MIRROR_DURATION_SUSPECT` | `worker:git-state-mirror.duration-suspect` | `i` | `git-state-mirror-worker-entry.ts::settleDurationProbe` (BUG-0005 R5) — a single recompute/status duration exceeded the 30 s sanity ceiling on BOTH the wall and monotonic clocks, i.e. the process was suspended mid-measurement (system sleep / App Nap) rather than git being slow. The sample is discarded (reported as 0 = "no measurement") instead of pinning the reconcile heartbeat at its 60 s cap. Field evidence: `lastStatusMs` medians of ~925 s for repos whose true median is 44–79 ms. Tagged `repoRoot`, `phase` (`recompute`/`recompute-failed`/`reconcile`), `wallMs`, `monotonicMs`, `ceilingMs`. Rate control: threshold-crossing only — the healthy path emits nothing. |
...
| `WORKER_GIT_STATE_MIRROR_SHUTDOWN_QUIESCED` | `worker:git-state-mirror.shutdown-quiesced` | `i` | `git-state-mirror-worker-entry.ts:shutdownWorker` — emitted after the native-quiesce barrier WITH settle re-validation (zero live subscriptions + zero pending unsubscribes + zero call-time-tracked in-flight native ops) just before `parentPort.close()`. Tagged `activeSubscriptions`, `pendingUnsubscribes`, `pendingNativeOps`, `settledMs`, `requiesceCount`, `deadlineHit`. The teardown-SIGABRT fix v2 (2026-07-23): the old two-counter barrier missed in-flight subscribes. |
| `WORKER_GIT_STATE_MIRROR_NATIVE_OP_REFUSED` | `worker:git-state-mirror.native-op-refused` | `i` | `git-state-mirror-worker-entry.ts::runGatedNativeWatcherOp` — a native watcher op was refused because shutdown already began; the gate and the binding call share one synchronous tick (closes the .gitignore-read async gap). Tagged `label`. |
| `WORKER_GIT_STATE_MIRROR_SHUTDOWN_REQUIESCE` | `worker:git-state-mirror.shutdown-requiesce` | `i` | `shutdownWorker` — the settle window ended non-quiescent (e.g. a subscribe resolved mid-settle and queued its compensating unsubscribe) and the barrier re-entered. Tagged `requiesceCount`, `spunMs`. Absence = clean single-pass quiesce. |
| `WORKER_GIT_STATE_MIRROR_UNCAUGHT_EXCEPTION_DRAIN` | `worker:git-state-mirror.uncaught-exception-drain` | `i` | Worker `uncaughtException`/`unhandledRejection` handler — drains native watcher ops through the quiesce barrier (capped 10 s) before `process.exit(1)`; previously the env tore down with live ops = mid-session whole-process SIGABRT. Tagged `source`, `message`. |
| `MAIN_APP_WILL_QUIT_CLEANUP_AWAITED` | `main:app.will-quit-cleanup-awaited` | `i` | `index.ts` will-quit handler — first pass prevented default and awaited `cleanupIpcHandlers()` (capped 17 s) before re-entering quit; the un-awaited cleanup was a barrier-bypass teardown path. Tagged `waitedMs`, `timedOut`. |
...
| `MAIN_APP_QUIT_CLEANUP_STEP_FAILED` | `main:app.quit-cleanup-step-failed` | `i` | `ipc-handlers.ts:runCleanupIpcHandlers` — a subsystem `dispose()` threw during quit-time cleanup; the chain logs + continues instead of aborting (a throwing ripgrep dispose used to skip the GitStateMirror drain and hang the quit to the debug-quit hard-exit floor). Tagged `step`, `error`. |
| `MAIN_QUIT_ACTIVITY_SCAN` | `main:quit.activity-scan` | `i` | `quit-activity-scan.ts:collectQuitActivitySummary` — the activity-aware quit confirmation enumerated shell descendants (once per quit dialog; fail-open). Tagged `terminalCount`, `activeCount`, `jobs` (≤3), `durationMs`, `outcome` (`ok`/`no-terminals`/`error`), `error?`. |
| `MAIN_SESSION_LEDGER_ABNORMAL_EXIT` | `main:session-ledger.abnormal-exit-detected` | `i` | `session-ledger.ts:initializeSessionLedger` — the previous instance left no clean-shutdown mark (SIGKILL / power loss / freeze force-quit: the death class with no crash report). Tagged `kind` (`abnormal`/`corrupt`), `lastPid`, `lastAppVersion`, `lastSeenAt`, `uptimeMs`, `terminatedActiveJobs`. |
| `MAIN_SESSION_LEDGER_MARKED_CLEAN` | `main:session-ledger.marked-clean` | `i` | `session-ledger.ts:markSessionLedgerClean` — a graceful-quit path (quit / debug-quit / restart-to-update) marked this session's ledger clean. Tagged `quitReason`, `terminatedActiveJobs`. |
...
| `MAIN_GIT_AUTOFETCH_SUCCEEDED` | `main:git-autofetch.succeeded` | `X` (duration) | `git-autofetch-manager.ts::reportResult` — fetch exited 0; triggers the repo revalidate that re-reads `# branch.ab`. Tagged `repoRoot`, `durationMs`, `freshnessFanoutCount` (how many subscribed cwds received the fetch-freshness delta — 0 distinguishes "published to nobody" from "published but not applied", which are otherwise indistinguishable in a bundle). |
| `MAIN_GIT_AUTOFETCH_FAILED` | `main:git-autofetch.failed` | `X` (duration) | `git-autofetch-manager.ts::recordFailure` — fetch failed. **BUG-0005**: stderr is now classified on EVERY path including the timeout kill (the timeout branch used to short-circuit past `classifyFetchFailure` and discard stderr, which is why 6 of 9 failures in a 98.8 h field bundle were undiagnosable). Tagged `repoRoot`, `reason` (`timeout`/`auth`/`no-remote`/`network`/`other`), `durationMs`, `killedByTimeout`, `classified` (what git was complaining about, even when killed), `exitCode`, `freshnessFanoutCount`, `remoteScheme` (transport CLASS only — `ssh`/`scp-like`/`https`/`http`/`file`/`git`/`none`/`other`; never the URL, host or user), `stderrTail` (≤240 chars, userinfo stripped and token-shaped runs collapsed by `sanitizeGitStderr`). |
...
| `MAIN_GIT_AUTOFETCH_SKIPPED_HIDDEN` | `main:git-autofetch.skipped-hidden` | `i` | `git-autofetch-manager.ts::recordHiddenSkip` — ticks are producing no fetches because the window is hidden/minimized (the confirmed hidden-pause strategy). **Rate control (BUG-0005)**: emitted on a CHANGE of the owed-work signature, or at most every 10 min — it previously fired every 30 s with an empty payload, so a 4-day bundle carried 935 identical `{}` records. Tagged `repoCount`, `overdueCount`, `neverFetchedCount`, `maxOverdueMs` (all finite; a never-attempted repo is counted rather than contributing an infinite age). |
...
| `MAIN_GIT_AUTOFETCH_PRIORITY_RETRY` | `main:git-autofetch.priority-retry` | `i` | `git-autofetch-manager.ts::handleRepoFocused` (BUG-0005 R1-A) — the user focused a Task whose repo was backed off, so one backoff-bypassing fetch was granted (or refused). Tagged `repoRoot`, `granted`, `reason` (`in-flight`/`cooldown`/`attempted-recently`/`already-pending`). **Rate control**: only GRANTS and rate-limited refusals are emitted; the two steady-state refusals — `not-backed-off` (every healthy repo) and `unknown-repo` (every repo not yet synced, or excluded for having no upstream) — are deliberately NOT emitted because they fire on every Task focus (a 30 s test slice produced 36 such records and zero useful ones). Grants inherit the scheduler's one-per-repo-per-5-min ceiling. |
| `MAIN_GIT_AUTOFETCH_STREAK_HALVED` | `main:git-autofetch.streak-halved` | `i` | `git-autofetch-manager.ts::setAppVisible` (BUG-0005 R1-B) — a hidden → visible EDGE halved every repo's failure streak, so failures accrued while paused do not keep the user pinned at the 1 h ceiling. Tagged `halvedRepoCount`. Emitted only on a real transition (the host fires setAppVisible on show/hide/minimize/restore) and only when something was actually backed off. |
| `MAIN_GIT_FETCH_USER_REQUESTED` | `main:git-fetch.user-requested` | `i` | `git-autofetch-manager.ts::fetchNow` — the user clicked the Task branch badge to sync. Distinguishes user-driven from scheduled fetches in the `succeeded`/`failed` events that follow (those carry `source: 'manual'`), and proves the recovery path was reachable — before it existed the app had NO manual fetch affordance anywhere. Tagged `repoRoot`. Rate control: one per click, and the handler no-ops while a fetch for that repo is in flight. |
...
| `RENDERER_TERMINAL_TITLE_SYNC_RENDERED` | `renderer:terminal-title.sync-rendered` | `i` | `src/components/TerminalGrid/TerminalGrid.tsx` — **BUG-0005 P1**, DIAGNOSTIC tier. `branch-rendered` above is opt-in tier AND fires only when the branch STRING changes, so an ahead/behind-only move — the entire subject of BUG-0005 — was invisible even with the opt-in tier on. Tagged `terminalId`, `cwd`, `ahead`, `behind`, `syncKind` (`no-upstream`/`never-attempted`/`never-succeeded`/`synced`). Rate control: emitted only when the rendered `(ahead, behind, stale)` triple actually changes. |
| `RENDERER_GIT_SYNC_FETCH_CLICKED` | `renderer:git-sync.fetch-clicked` | `i` | `src/components/TerminalGrid/TerminalGrid.tsx::handleBranchSync` — the badge click fired. Paired with the main-side `main:git-fetch.user-requested`: this present + that absent means the click reached the renderer but never crossed IPC. Diagnostic tier (default-on) — a recovery path nobody can prove was taken is not a recovery path. Tagged `cwd`. Rate control: user-gesture frequency, and the handler returns early while a fetch is in flight. |
...
| `RENDERER_GIT_DIFF_MODEL_SYNC` | `renderer:git-diff.model-sync` | `X` (duration) | Same file — selected-file content, editor mount, or state change reconciled the live Monaco original/modified models against renderer `fileContentsRef` so stable model URIs cannot show stale bodies. **Promoted to `perfTraceDiagnostic` (prod default-on) on 2026-07-27**: "did the click actually write into the models, and with what lengths" was the single link the 2026-07-26 bundle could not answer, because this event rode the opt-in tier. Selection / model-reconcile frequency with a scalar payload — not a hot path. Tagged `cwd`, `terminalId`, `fileKey`, `filename`, `changeType`, `reason`, `result`, `originalChanged`, `modifiedChanged`, `originalLen`, `modifiedLen`, `durationMs`. |
| `RENDERER_GIT_DIFF_RESTORE_DECISION` | `renderer:git-diff.restore-decision` | `i` | Same file (`restoring-scroll` layout effect + the deferred branch in `onDidUpdateDiff`) — the render-then-reveal cycle's restore-vs-reveal decision (`resolveDiffRestoreDecision` in `diffViewMemory.ts`): restore the saved view state when the content signature still matches what the user last saw, otherwise land on the first change (VS Code `textDiffEditor` precedence + the content-changed override). `trigger` records what advanced `waiting-diff` → `restoring-scroll`: `warm-ready` = warm-reopen fast path, `diff-computed` / `model-bound` = normal signals, `deferred-diff-computed` = reveal parked until Monaco computed the diff, `timeout` = the cycle idled the full 2 s safety window — the 2026-07-18 "stale until manual refresh" bundle signature. `revealTargetLine` carries the applied reveal's target line (null when no reveal ran — the autotest gate for GDS-52). Emitted via `perfTraceDiagnostic` (default-on in prod; reopen/file-switch frequency). Tagged `cwd`, `terminalId`, `action`, `reason`, `trigger`, `revealTargetLine`, `filename`. |
| `RENDERER_GIT_DIFF_WARM_READY_GATE` | `renderer:git-diff.warm-ready-gate` | `i` | `src/components/GitDiffViewer/GitDiffViewer.tsx` (`maybeCompleteWarmReveal`, emitted ONCE when the gate opens — never per rAF tick). The warm-reopen fast path used to open on `editor.getLineChanges() !== null`, which answers "has a diff ever been computed on this widget" rather than "does the computed diff describe the models bound right now"; Monaco keeps `_diff` across a content change and only debounces the recompute for 200 ms, so the old gate stayed open through the whole window in which its answer was wrong (2026-07-26 bundle, BUG-0004). `diffCurrent` is the field that matters — `diffCurrent:false` on a warm-ready decision is the defect signature and must not occur post-fix. Diagnostic tier (prod default-on), reopen / file-switch frequency. Tagged `cwd`, `terminalId`, `filename`, `changeType`, `lineChangesLen`, `diffCurrent`, `waitedMs`. |
| `RENDERER_GIT_DIFF_COLLAPSE_STATE` | `renderer:git-diff.collapse-state` | `i` | Same file (debounced 250 ms after a settled reveal; deliberately NOT wired to `onDidContentSizeChange`, which fires per layout pass). Monaco's `hideUnchangedRegions` outcome, measured by comparing the laid-out height against the model's fully-expanded height — there is no public getter for hidden ranges. Exists because "was the file actually collapsed" was invisible in every trace before it: the 2026-07-26 bundle could only reason from source about the user's report that the whole file rendered expanded. Monaco carries the PREVIOUS diff's region visibility forward (`updateUnchangedRegions`' "Transfer state from cur state"), so a base whose diff had zero unchanged regions used to leave every later region expanded. Tagged `cwd`, `terminalId`, `filename`, `changeType`, `totalLines`, `laidOutLines`, `hiddenLineCount`, `collapsed`. |
| `RENDERER_GIT_DIFF_MODEL_IDENTITY_REUSED` | `renderer:git-diff.model-identity-reused` | `i` | Same file (`handleEditorDidMount`, transition only). A model URI was bound whose base identity differs from the one the current selection expects. Post-fix this cannot happen — the identity is part of both the URI and the React key — so it is a tripwire: a regression of the identity scheme becomes visible in a user's trace instead of surfacing only as a mis-positioned viewport. Tagged `cwd`, `terminalId`, `filename`, `changeType`, `boundOriginalUri` (tail 120), `expectedOriginalUri` (tail 120). |
| `RENDERER_GIT_DIFF_MODEL_SWEEP` | `renderer:git-diff.model-sweep` | `i` | Same file (`sweepSupersededDiffModels`, emitted only when something was actually disposed). Content-identity URIs mean every new base mints a fresh model pair, so without a sweep a coding agent editing the tree would grow Monaco's model table for the life of the session; the sweep is also what keeps the stable-URI worktree side honest, since a superseded model must be gone before the next mount rebuilds it from current content. Tagged `cwd`, `terminalId`, `reason` (`bind` \| `detach`), `disposed`, `live`, `retained`. |
| `RENDERER_GIT_DIFF_VIEW_MODEL_REBUILT` | `renderer:git-diff.view-model-rebuilt` | `i` | `src/components/GitDiffViewer/GitDiffViewer.tsx` (`syncCurrentDiffEditorModels`, once per write that actually changed a side). A full-content `setValue` takes TextModel's flush path, which destroys every decoration on the model — including the ones Monaco tracks unchanged-region positions with — so the next `updateUnchangedRegions` resolves them all to undefined, inverts an EMPTY hidden set into "the whole file is visible", and hideUnchangedRegions silently stops collapsing (the 2026-07-26 bundle's whole-file-expanded symptom, reached by a second route). Rebuilding the view model via `setModel` with the same pair resets `_unchangedRegions` so the next computation derives regions from scratch. Tagged `cwd`, `terminalId`, `filename`, `changeType`, `reason`, `originalChanged`, `modifiedChanged`. |
| `RENDERER_GIT_DIFF_REVEAL_RECONCILE` | `renderer:git-diff.reveal-reconcile` | `i` | `src/components/GitDiffViewer/GitDiffViewer.tsx` (`onDidUpdateDiff`, only when the resolved action is not `none`). The applied viewport position records which content it was computed from; when that record no longer matches what is loaded, this event says how it converged: `wait` (Monaco's diff does not yet describe the bound models), `reconcile-silent` (repositioned without disturbing the user), `notify` (the user had scrolled, so the viewport is theirs and it was surfaced instead of moved). Exists because staleness became a state that converges rather than an instant that had to be got right — the trace should show which path each convergence took, and a run with no `reconcile-silent` at all under a mutating tree would mean convergence is not running. Tagged `cwd`, `terminalId`, `filename`, `changeType`, `action`, `userOwnsViewport`, `appliedReason`. |
| `RENDERER_GIT_DIFF_VIEWPORT_GOAL` | `renderer:git-diff.viewport-goal` | `i` | Same file — lifecycle of the viewport-goal self-heal: a restore/reveal applied against a not-yet-measured diff layout is CLAMPED by Monaco (fresh remounts report scrollHeight ≈ viewport for several frames; `hideUnchangedRegions` layout also settles late), so the intended goal parks and `onDidContentSizeChange` re-applies it. `phase`: `parked` (goal registered because the first apply was clamped/short), `satisfied` (target honored), `cancelled-scroll` (user scrolled away), `expired` (10 s window), `cancelled-context` (file/editor changed). Emitted on transitions only, never per size event. Diagnostic tier (prod default-on), reopen frequency. Tagged `cwd`, `terminalId`, `phase`, `kind`, `scrollTop`, `line`. |
| `RENDERER_GIT_DIFF_CACHE_INVALIDATION` | `renderer:git-diff.cache-invalidation` | `i` | Same file — renderer received a backend Git Diff cache invalidation and either marked visible file bodies stale for background refresh or cleared closed-view caches. Since 2026-07-12 emitted via `perfTraceDiagnostic` (default-on in prod; mirror-invalidation-scale frequency): whether the renderer received/matched the push is the load-bearing breadcrumb for staleness bundles. Tagged `cwd`, `terminalId`, `invalidatedCwd`, `reason`, `isOpen`, `retainedEntries`, `staleEntries`. Since 2026-07-16 (revert-scope fix) a file-scoped known mutation adds `scopedFiles` (count) and marks ONLY the listed files stale — the branch fires NO reload of its own (the mutation initiator patched locally; mirror echo / timer fallback reconciles). |
| `RENDERER_GIT_DIFF_LOCAL_PATCH_APPLIED` | `renderer:git-diff.local-patch-applied` | `i` | `src/components/GitDiffViewer/GitDiffViewer.tsx` — optimistic local list patch after a known single-file mutation (stage/unstage/discard). `patched:false` = the conservative table (`gitDiffLocalPatch.ts`) refused the case and the caller fell back to one silent reload — the load-bearing breadcrumb when a "row didn't update after discard" bundle arrives. Tagged `action`, `filename`, `changeType`, `patched`. User-action frequency (2026-07-16 revert-scope fix). |
| `RENDERER_GIT_DIFF_MUTATION_RECONCILE_FALLBACK` | `renderer:git-diff.mutation-reconcile-fallback` | `i` | Same file — the 3 s post-mutation reconcile fallback fired because NO invalidation-driven reload arrived (mirror echo missed/late). Rare; its presence says the watcher path went quiet after a known mutation. Tagged `cwd`. |
...
| `MAIN_GIT_DIFF_CONTENT_CACHE_INVALIDATE_PROJECT` | `main:git.diff.content-cache.invalidate-project` | `i` | Same file — `gitDiffCacheInvalidator` fired. For `reason='mirror'` (FS-watcher churn) the bucket is re-validated per file (`scoped:true`, also tags `keptEntries`) — only files whose working-tree stat changed are evicted, keeping unrelated files warm (kar-qemu content-cache thrash fix). Since 2026-07-16 (revert-scope fix) a KNOWN single-file mutation (stage/unstage/discard/save) evicts per-file too via `evictContentCacheEntriesForFiles` (`scoped:true` + `fileCount`); only un-scoped mutations / force / LRU still wipe the whole bucket (`scoped:false`). Tagged `project`, `reason`, `droppedEntries`, `scoped`, `keptEntries`, `fileCount`. |
...
| `RENDERER_GIT_DIFF_OPEN_PHASE_FIRST_PAINT` | `renderer:git-diff.open-phase.first-paint` | `X` | Same emitter — double-rAF after the list applied: React committed and a frame presented, i.e. the moment the user actually stops seeing the open spinner. `durationMs` spans request→paint. Tagged `cwd`, `fileCount`, `durationMs`, `superseded` (true when a newer load replaced this token before its paint frame — previously the event was silently swallowed on supersede, leaving rapid-invalidation sessions with no first-paint at all; Electron 43 upgrade fix). |
...
| `RENDERER_PROJECT_EDITOR_HTML_SCROLL_APPLY` | `renderer:project-editor.html-scroll-apply` | `i` | `HtmlReader.tsx::applyPendingScrollRestore` — outcome of each scroll-restore apply attempt (first apply + bounded re-pins). Electron 43 (Chromium 150): the custom-protocol document clamps scrollTo during layout settle, resets scroll on late load work, and loses scroll across detach/reattach of a soft-closed editor — this event shows what each apply actually achieved vs the target. Tagged `attempt`, `targetY`, `appliedY`, `converged`. |
| `RENDERER_PROJECT_EDITOR_HTML_ZOOM_RESTORED` | `renderer:project-editor.html-zoom-restored` | `i` | `ProjectEditor.tsx` (ready+loadCount-gated zoom-apply effect) — the remembered HTML preview zoom was re-applied to the current document: on (re)open once the iframe session became ready, AND after every in-place document swap (toolbar / Cmd+R reload, link navigation, back / forward / home) since those keep `ready` true while resetting the fresh document's zoom to 1. Breadcrumb for "zoom reverts to 1 after a subpage round trip / reload / navigation". Diagnostic channel. Tagged `zoomFactor`, `loadCount`. |
| `RENDERER_PROJECT_EDITOR_HTML_ANCHOR_SCROLL` | `renderer:project-editor.html-anchor-scroll` | `i` | `HtmlReader.tsx` (`anchor-scroll` bridge message) — the HTML preview bridge intercepted an in-page anchor click (raw `href` starting with `#`) and performed a programmatic `scrollIntoView` inside the frame (pure scroll: no host navigation, no history entry, works at any CSS zoom because it never relies on Chromium's fragment navigation). Breadcrumb for "clicking a TOC entry does not jump" reports. Diagnostic channel. Tagged `hash` (≤64 chars), `found`. |
| `RENDERER_PROJECT_EDITOR_HTML_SAME_URL_RELOAD` | `renderer:project-editor.html-same-url-reload` | `i` | `HtmlReader.tsx` (`navigate-request` same-URL guard) — a link click resolved to the URL the iframe src already carries; React would skip the attribute write, so instead of a dead push (which latched `isLoading` and stuck the loading overlay) the host reloads the document in place. Diagnostic channel. No payload beyond `ph`. |
| `MAIN_HTML_PREVIEW_LINK_CLASSIFIED` | `main:html-preview.link-classified` | `i` | `ipc-handlers.ts` (`HTML_PREVIEW_CLASSIFY_NAVIGATION`) — an HTML Preview link click was classified into its navigation route (`external` / `in-frame` / `project-file` / `outside-root` / `invalid`) by `classifyHtmlPreviewLink`. Breadcrumb for "clicking a link broke the preview / opened the wrong surface" — shows exactly what the router saw before the renderer acted. Diagnostic channel. Tagged `kind`, `ext` (≤16 chars, file routes only). |
| `RENDERER_PROJECT_EDITOR_PREVIEW_LINK_OPEN_FILE` | `renderer:project-editor.preview-link-open-file` | `i` | `HtmlReader.tsx` (`navigate-request` project-file branch) + `ProjectEditor.tsx::handleMarkdownPreviewLinkClick` — a link inside the HTML / Markdown preview resolved to a local project file and was dispatched to the Project Editor's viewer stack (`openFileFromPreviewLink`) instead of navigating the preview (the iframe cannot render non-HTML files: octet-stream + sandboxed download). Happy-path breadcrumb of the "open in matching viewer" feature. Diagnostic channel. Tagged `sourceKind` (`html` / `markdown`), `ext` (≤16 chars). |
| `RENDERER_PROJECT_EDITOR_PREVIEW_LINK_BLOCKED` | `renderer:project-editor.preview-link-blocked` | `i` | `HtmlReader.tsx` (`navigate-request` refusal branch) + `ProjectEditor.tsx::handlePreviewLinkBlocked` — a preview link was refused (`outside-root` / `invalid:*` / `unsupported-protocol`) and surfaced as a status message instead of the previous silent failure. Rejection-branch breadcrumb. Diagnostic channel. Tagged `sourceKind`, `reason` (≤64 chars). |
| `RENDERER_PROJECT_EDITOR_PREVIEW_LINK_RETURN` | `renderer:project-editor.preview-link-return` | `i` | `ProjectEditor.tsx::handleBackToPreviewSource` — the one-click return bar jumped back to the preview file a link came from (scroll restores through the existing `FileViewMemory` path). Closes the link round-trip evidence chain. Diagnostic channel. Tagged `sourceKind`. |
| `RENDERER_PROJECT_EDITOR_OPEN_CHOICE_SYSTEM_APP` | `renderer:project-editor.open-choice-system-app` | `i` | `ProjectEditor.tsx::openFileInSystemApp` — the open-choice dialog (or a remembered `system` default) routed a file with no in-app viewer to the OS default application via `shell.openPath`. Diagnostic channel. Tagged `ext`, `remembered`. |
| `RENDERER_PROJECT_EDITOR_PREVIEW_LINK_EXTERNAL_TO_BROWSER` | `renderer:project-editor.preview-link-external-to-browser` | `i` | `ProjectEditor.tsx::openExternalUrlInBrowserPanel` — an external http(s) preview link handed off to the Open Browser panel: the editor soft-closes with full view retention, then `browser:open` fires with `fromEditor` armed. The fix for "external link white-screens the iframe" (CSP frame-ancestors). Diagnostic channel. Tagged `host` (≤128), `urlLen`. |
| `RENDERER_PROJECT_EDITOR_PREVIEW_LINK_EXTERNAL_PROTOCOL` | `renderer:project-editor.preview-link-external-protocol` | `i` | `ProjectEditor.tsx::openExternalProtocolLink` — a mailto:/tel: preview link (HTML or Markdown source) routed to the OS default handler through the confirm-gated `shell.openExternal`. Diagnostic channel. Tagged `protocol`, `sourceKind`. |
| `RENDERER_TERMINAL_GRID_BROWSER_RETURN_TO_EDITOR` | `renderer:terminal-grid.browser-return-to-editor` | `i` | `TerminalGrid.tsx::handleCloseBrowser` — closing an editor-initiated Open Browser session consumed the one-shot `fromEditor` flag and reopened the Project Editor (`project-editor:open`), completing the external-link round trip. Diagnostic channel. |
...
| `RENDERER_EPUB_HOST_KEY_FORWARDED` | `renderer:epub.host-key-forwarded` | `i` | `EpubReader.tsx` rendition `keydown` relay — a keydown inside the epub.js content iframe matched the shared reader host-key allowlist (Escape / Cmd+P, `src/utils/readerHostKey.ts`) and was re-dispatched on the host document so `useSubpageEscape` / Quick Open fire. Breadcrumb for "ESC doesn't leave the EPUB reader". Diagnostic channel. Tagged `surface`, `key`, `metaKey`, `ctrlKey`. |
| `RENDERER_PROJECT_EDITOR_OUTLINE_TRUNCATED` | `renderer:project-editor.outline-truncated` | `i` | `useOutlineSymbols.ts` post-parse — the parsed outline exceeded `OUTLINE_SYMBOL_CAP` (5000) and was depth-truncated before entering React state (2026-07-19 subpage CPU storm fix). Breadcrumb for "outline is missing deep symbols" and for sizing pathological documents. Tagged `totalCount`, `keptCount`, `filePath` (sliced). |
| `RENDERER_PROJECT_EDITOR_OUTLINE_VIRTUALIZATION` | `renderer:project-editor.outline-virtualization-mode` | `i` | `OutlinePanel.tsx` — the visible-row count crossed `OUTLINE_VIRTUALIZE_THRESHOLD` (300) and the panel switched to (or left) fixed-row windowed rendering; deduped per mode flip. Proves in a user trace whether a giant outline was windowed or fully materialised. Tagged `rowCount`, `virtualized`. |
| `RENDERER_TERMINAL_GRID_SUBPAGE_FREEZE` | `renderer:terminal-grid.subpage-subtree-freeze` | `i` | `SubpageSubtreeFreeze.tsx` — a soft-closed subpage panel's React subtree entered/left the frozen state (element-identity bailout while hidden; the fix that stops TerminalGrid churn from reconciling a hidden 40k-item editor tree). Breadcrumbs for "hidden panel kept re-rendering" and stale-props-after-reopen regressions. Tagged `panel` (`editor` / `diff` / `history`), `frozen`. |
...
| Constant | Name | Phase | Call site |
|---|---|---|---|
| `MAIN_PTY_SPAWN` | `main:pty.spawn` | `X` | `electron/main/pty-manager.ts` `PtyManager.create()`. Payload includes `integrationMode` (`ps-inline-command` / `bash-rcfile` / `zsh-zdotdir` / `fish-vendor-conf` / `cmd-prompt-env` / `none`) and a FIXED `hasShellIntegration` (= mode ≠ none; the old flag counted env keys only, so args-based PS injection always read `false` — the misleading field that cost diagnosis time on the 2026-07 bundles). |
...
| `RENDERER_PROMPT_EDITOR_CTX_MENU_SCROLL_DISMISS` | `renderer:prompt.editor.ctx-menu-scroll-dismiss` | `i` | `PromptEditorContextMenu.tsx` — the open prompt context menu was dismissed because a container that can move its anchor scrolled (`shouldDismissMenuOnScroll` in `popup-position.ts` returned true). Payload carries `ignoredScrolls`: the count of unrelated-container scrolls (e.g. terminal viewport auto-follow while a Task streams output) the menu survived while open — those are aggregated, never traced per event, because they arrive per rendered frame on the output hot path. A bug report "menu closed by itself" with a trace shows whether the dismiss branch fired and how much background scrolling preceded it. |
...
| `RENDERER_PROMPT_SENDER_LAYOUT_SYNC` | `renderer:prompt.sender.layout-sync` | `i` | `PromptSender.tsx` — Task selector re-derived its grid geometry from the active Task layout. Emitted only when the resulting shape changes (layout switch, custom-preset edit, Task count change), never per render. Payload: `kind` (`preset`\|`custom`), `columns`, `rows`, `slots`, `signature` |
...
| `RENDERER_TASK_REORDER_MODE_ENTER` | `renderer:task-reorder.mode-enter` | `i` | `useTaskRearrange.ts::enterMode` — the grid armed drag-to-rearrange. Tagged `trigger: 'long-press' \| 'menu'` so a report of "my Tasks reordered themselves" shows whether the user held a title or used the dropdown. |
| `RENDERER_TASK_REORDER_MODE_EXIT` | `renderer:task-reorder.mode-exit` | `i` | `useTaskRearrange.ts::exitMode` / long-press drop path. Tagged `reason: 'esc' \| 'outside-click' \| 'drop' \| 'layout-change' \| 'tab-switch' \| 'terminal-count-change' \| 'disabled'`. The force-exit reasons are the breadcrumb for "the grid got stuck masked": an enter with no matching exit means a leaked mode. |
| `RENDERER_TASK_REORDER_COMMIT` | `renderer:task-reorder.commit` | `i` (carries `durationMs`) | `useTaskRearrange.ts::endDrag` — a drop that actually changed the order, emitted around the AppState write. Tagged `fromIndex`, `toIndex`, `count`, `durationMs`. The ONLY event on the drag path: `pointermove` deliberately emits nothing (it runs every frame and only writes CSS transforms). |
| `RENDERER_TASK_REORDER_CANCEL` | `renderer:task-reorder.cancel` | `i` | `useTaskRearrange.ts::endDrag` — a drag that ended without reordering. Tagged `fromIndex`, `toIndex`, `reason: 'same-slot' \| 'aborted'`. Distinguishes "user changed their mind" from "the drop was swallowed", which look identical in a bug report. |
| `RENDERER_TASK_REORDER_REFIT` | `renderer:task-reorder.refit` | `i` (carries `durationMs`) | `TerminalGrid.tsx::handleRearrangeAfterCommit` — custom-layout-only post-commit `forceFit` over the moved span, because a Task can land in a differently sized rectangle and the PTY has to re-derive cols/rows. Preset layouts never emit this (identical slot geometry, `fit()` self-skips). Tagged `fromIndex`, `toIndex`, `refitted`, `durationMs`. |
| `RENDERER_MODAL_ESC_CANCELLED` | `renderer:modal-dismiss.esc-cancelled` | `i` | Unified modal dismiss policy — every ESC-driven modal cancel. Emitted by `src/hooks/useModalEscape.ts` (shared window bubble-phase listener used by CommandSelectModal, CodingAgentModal, DownsizeConfirmDialog, TabBar close-confirm, PromptNotebook send-history) and by the subpage-host escape chains that settle an open dialog before the subpage itself closes (`GitDiffViewer.tsx` / `GitHistoryViewer.tsx` large-file confirm, `ProjectEditor.tsx` dialog + file-open-choice). Tagged `dialog` naming the surface. Backdrop clicks on modals are inert by policy and emit nothing (no handler exists). |
...
| `MAIN_GPU_PROCESS_GONE` | `main:gpu.process-gone` | `i` | `electron/main/gpu-crash-recovery.ts::broadcastGpuProcessGone` (driven by `app.on('child-process-gone')`, type GPU) — the Electron 39 ANGLE-Metal Space-switch crash trigger, now visible inside the trace instead of only in `~/Library/Logs/DiagnosticReports`. Payload `{ reason, exitCode, simulated, msSinceLastWindowShow, msSinceLastWindowFocus, msSinceLastThrottleToggle, msSinceLastNudge }` — the msSince* adjacency fields are the BUG-0003 P0 crash-antecedent correlation (wake +30 ms / throttle-toggle +14 ms were previously manual timeline reconstruction). |
| `RENDERER_XTERM_RENDERER_GPU_CRASH_RECOVERY` | `renderer:xterm.renderer.gpu-crash-recovery` | `i` | `terminal-session-manager.ts::executeGpuCrashRecovery` — two-phase rebuild of every visible pane's WebGL after the GPU-gone broadcast (webglcontextlost is NOT delivered on this crash path). Payload `{ sessionCount, recreatedCount, failedCount, reason, simulated, twoPhase, wasDeferred }`. |
| `RENDERER_XTERM_RENDERER_GPU_CRASH_RECOVERY_DEFERRED` | `renderer:xterm.renderer.gpu-crash-recovery-deferred` | `i` | `terminal-session-manager.ts::recoverRendererSurfacesAfterGpuCrash` — broadcast arrived while the document was hidden; rebuild deferred to the next document-visible (BUG-0003: a hidden-document rebuild cannot be paint-verified). Payload `{ reason, simulated }`. |
| `RENDERER_XTERM_ATLAS_RECOVERY_RENEWAL` | `renderer:xterm.atlas.recovery-renewal` | `i` | `terminal-session-manager.ts::executeGpuCrashRecovery` — dispose phase completed across ALL panes, shared glyph-atlas owner count hit zero, poisoned cache entry evicted before any recreate (BUG-0003 root of "tab switch cannot self-heal"). Payload `{ disposedCount }`. |
| `RENDERER_XTERM_RENDERER_GPU_CRASH_STICKY_FALLBACK` | `renderer:xterm.renderer.gpu-crash-sticky-fallback` | `i` | `terminal-session-manager.ts::engageGpuCrashStickyFallback` — the session GPU-crash fuse blew (2nd crash, N=2 product decision): every terminal disposed WebGL and sticks to the DOM renderer until restart; TabBar banner raised. Once per session. Payload `{ crashCount, disposedCount, reason, simulated }`. |
