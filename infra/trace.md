...
| `RENDERER_TERMINAL_TITLE_SYNC_RENDERED` | `renderer:terminal-title.sync-rendered` | `i` | `src/components/TerminalGrid/TerminalGrid.tsx` — **BUG-0005 P1**, DIAGNOSTIC tier. `branch-rendered` above is opt-in tier AND fires only when the branch STRING changes, so an ahead/behind-only move — the entire subject of BUG-0005 — was invisible even with the opt-in tier on. Tagged `terminalId`, `cwd`, `ahead`, `behind`, `stale`. Rate control: emitted only when the rendered `(ahead, behind, stale)` triple actually changes. |
