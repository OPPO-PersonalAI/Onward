...
# PDF text-selection engine autotest (Windows parity of
# run-pdf-text-selection-autotest.sh).
#
...
$RepoRoot = $RootDir
if (-not $LogFile) {
  $LogFile = Join-Path $RepoRoot 'traces/test-logs/pdf-text-selection-autotest.log'
}
...
$RunTmpDir = Join-Path ([IO.Path]::GetTempPath()) ("onward-pdf-textsel-" + [Guid]::NewGuid().ToString('N'))
...
try {
  ...
  Write-Host "Starting PDF text-selection autotest..."
  ...
  $env:ONWARD_AUTOTEST_SUITE = 'pdf-text-selection'
  ...
  if ($content -match '\[AutoTest\] FAIL') {
    Write-Host "PDF text-selection autotest FAILED" -ForegroundColor Red
    Write-Host ""
  ...
  if ($content -notmatch 'suite-done:PdfTextSelection') {
    Write-Host "PDF text-selection autotest did not complete" -ForegroundColor Yellow
    Get-Content $LogFile -Tail 40
    exit 1
  }

  if ($content -match 'totalFailed:\s+[1-9]') {
    Write-Host "PDF text-selection autotest reported failed cases in the summary" -ForegroundColor Red
    exit 1
  }

  Write-Host "PDF text-selection autotest PASSED" -ForegroundColor Green
  Write-Host "  Log: $LogFile"
} finally {
