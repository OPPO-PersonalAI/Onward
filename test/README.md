...
| Bug fix (RC-2 2026-07): a git repo probe killed at the exec budget (hanging network volume) is classified `timeout` — NOT "not a repo" — with an exponential-backoff negative cache (30 s → 2 min → 5 min) in both main (`getGitRepoMeta`) and the GSM worker, and a distinct Git Diff empty state with retry | `test/unittest/git-meta-cache-policy.test.mts` (classifier decision table + backoff ladder; pure logic — the hang itself has no deterministic E2E fixture, see §3 timeout-triage) |
