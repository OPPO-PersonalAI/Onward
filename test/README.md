...
| PDF / EPUB compare in Diff + Git History (added / deleted / modified, single-pane collapse) + PDF **annotation diff panel** (three-state added/removed/changed counts from the two panes' CYY_MARK broadcasts, in-pane dashed emphasis, row click scrolls the target pane; fixture pair differs in annotations ONLY — `pdf-annotation-diff-fixture-builder.mjs`) | `run-pdf-epub-diff` (`git-diff-pdf-*` incl. `git-diff-pdf-annotation-*`, `git-diff-epub-*`, `git-history-pdf-*`, `git-history-epub-*`) + `test/unittest/pdf-annotation-diff.test.mts` (PAD-U-01..17: three-state model, per-field change detection, label-rename ≠ annotation-change, jump-pane targeting, emphasis sets) |
| PDF external-change auto-refresh — an agent rewrites the open PDF on disk: in-place viewer reload preserving page/zoom/scroll, externally added annotations adopted, the annotation autosave **never** triggers a self-reload (fingerprint suppression across the atomic-replace rename path, 5x aggregate), a half-written file defers silently then recovers (SumatraPDF semantics), unsaved local edits rebase-merge over the external bytes (local wins, conflicts counted), reopen after an external change is never stale (`v=mtime` cache-buster). E2E refresh budget **< 3 s** (user-agreed 2026-08-01, 1-of-3) | `run-pdf-external-refresh` (`pdf-extrefresh-*`; suite `src/autotest/test-pdf-external-refresh.ts`, throwaway mktemp project root seeded from the `pdf-text-selection` fixture) + `test/unittest/file-watch-binary-core.test.mts` (FWB-U-01..17: emit/skip decisions, self-write fingerprints, hash budget) + `test/unittest/pdf-reload-core.test.mts` (PRC-U-01..12: reload dedup, retry schedule, view-state clamp) + `test/unittest/pdf-annotation-merge-core.test.mts` (AMC-U-01..14: three-way rebase decision table) + `test/unittest/pdf-annotation-store.test.mts` (PAS-U-16..20: rebase entry point, external-modified gate reaction, mid-flight dirty preservation) + `test/unittest/pdf-preview-url.test.mts` (PPU-U-01..06: versioned file-URL derivation) + `test/unittest/watch-path-match.test.mts` (WPM-U-01..09: watcher-event ↔ active-file path matching — absolute activeFilePath and separator-run collapse, both shipped as real silent-drop bugs) |
...
| PDF highlight annotations — create from a selection, palette latency budget (**< 100 ms**, user-agreed 2026-07-29), note editing, deletion, the annotation list panel (renders in step with the viewer's own record count, label filter narrows and restores, sort preserves entries, list jump actually scrolls the viewer, list delete removes it from the viewer), custom zoom entry (applies / clamps out-of-range / rejects garbage and flags the field), and the persistence round trip that is the point of the feature: write annotations INTO the PDF, close, reopen, and assert records + notes + quad geometry all come back from the FILE, with no double-render from pdf.js's native layer. The suite MODIFIES the PDF it opens, so it works on a throwaway copy | `run-pdf-highlight` (`pdf-highlight-*` incl. `pdf-highlight-manage-*` for the manage-labels dialog: opens with built-ins read-only, custom label rename / recolor / delete round-trips real React state; suite `src/autotest/test-pdf-highlight.ts`, shares the `pdf-text-selection` fixture) + `test/unittest/pdf-highlight-geometry.test.mts` (PHG-U-01..20: rect→QuadPoints round trip, colour) + `test/unittest/pdf-annotation-store.test.mts` (PAS-U-01..20: when a save may and may not happen — R1/R3/R4/R5 protections + external-rebase entry) + `test/unittest/pdf-viewer-module-boundaries.test.mts` (PMB-U-01..05: cross-module state leaks, store-array reassignment, script load order) + `test/unittest/pdf-annotation-list-model.test.mts` (PAL-U-01..32: list ordering / filtering / note-expansion overrides / label validation + label management rename-recolor-delete, custom-only scope) |
| PDF outline auto-follow — the active outline entry tracks the SCROLL position, not just the page number (page-only matching sits on one heading for a whole page), survives a scroll without clearing, plus the locate button that re-centres after the user has browsed the list by hand | `run-pdf-epub-preview-pdf-outline` (`pdf-outline-auto-follow-survives-scroll`, `pdf-outline-locate-button-present`, `pdf-outline-locate-button-recenters`) + `test/unittest/pdf-annotation-list-model.test.mts` (PAL-U-20..26: the comparator — multi-section pages, headings still below the viewport, /Fit destinations with no position, out-of-order outlines) |
| PDF text selection inside the reader — drag selection, the `visible highlight === Selection.toString() === clipboard` agreement axiom (N=5), ligature integrity on a real render (`ffi` never torn), hidden/OCR text suppression (`Tr 3` text under visible text must be undraggable AND uncopyable), blocking annotations (a FreeText appearance must not leak the PDF text beneath it), click-clears-selection, zoom drops stale cross-page overlay, and selection still works after a document switch + reopen. Runner also gates on the fixture builder's `--check` and on `scripts/apply-pdfjs-patches.mjs --check`, so a lost pdf.js patch fails with a clear message instead of a confusing assertion | `run-pdf-text-selection` (`pdf-textsel-*`; suite `src/autotest/test-pdf-text-selection.ts`, fixture `test/autotest/fixtures/pdf-text-selection/onward-textsel.pdf` from `pdf-text-selection-fixture-builder.mjs`) + `test/unittest/pdf-text-selection-segmentation.test.mts` (PTS-U-01..15: grapheme / ligature units) + `test/unittest/pdf-text-selection-annotation-role.test.mts` (PTA-U-01..21: blocking vs passthrough, rect geometry) + `test/unittest/pdf-viewer-trace-relay.test.mts` (PVT-U-01..16: iframe trace relay + pdf.js patch integrity) |
