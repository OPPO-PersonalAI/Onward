...
| Memory diagnostics closed loop (Tier-1 sampling main/workers/renderer; pressure detector; memory report; notification; consented heap-snapshot bundle export) | `run-memory-watch` (MW-01..09 + post-exit trace-chunk asserts); unit `test/unittest/memory-pressure-detector.test.mts` (MPD-U-01..19: unit conversion, window/sustain semantics, prompt guards, snapshot headroom) |
