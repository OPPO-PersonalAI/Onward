...
| HTML source editing plus WebContents preview with local file assets, HTTP script access, persistent force refresh, splitter drag, WebContents search, HTML Preview zoom, browser-aligned refresh shortcut, scroll-preserving fresh reload, and persisted HTML scroll memory (`FileViewMemory.htmlScrollX/Y`: survives file-switch-and-back, ESC close→reopen, appState round-trip) | `run-project-editor-html-preview` (PHTML-00..19) + `test/unittest/app-state-file-states-validation.test.mts` (AFVM-U-*, persistence whitelist incl. the previously-dropped PDF/EPUB fields) |
