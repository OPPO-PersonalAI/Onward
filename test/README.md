...
| 8-grid (2x4) preset, Custom layout popover, downsize confirm dialog, focusTerminal 7/8 shortcuts. Downsize dialog follows the unified modal dismiss policy: backdrop click is inert (TLM-04b), ESC cancels via the shared `useModalEscape` hook (TLM-04c). **Task drag-to-rearrange (2026-07-28)**: press-and-hold anywhere on a Task's HEADER BAR for 300 ms (or the dropdown's Rearrange item) to arm the mode — the handle is the whole bar, not the title text, which is un-aimable inside a 24px strip that also holds the dropdown button and three metadata chips, then drag a Task onto another slot — insert-shift semantics (dragging slot 0 onto slot 3 of 6 yields `2,3,4,1,5,6`, NOT a swap), Task numbers follow POSITION so `Cmd+1` always means the first slot; visuals follow DESKTOP reorder convention (source dims to 40% and stays put, cursor-tracking ghost, drop-target ring, 700ms settle flash) rather than the iOS jiggle — terminal content stays readable throughout, and only VISIBLE slots participate (surplus Tasks stay parked at the array tail). The drag never mutates the terminal list — it previews via CSS transforms over a drag-start rectangle snapshot and writes AppState exactly once on drop, so no xterm remount and no per-frame PTY resize. TLM-09 is the load-bearing one: it injects a scrollback marker before the drag and reads it back after, proving the PTY survived. | `run-task-layout` (TLM-00..05 incl. TLM-04b/TLM-04c; TLM-03-grid-layout-eight-appstate-grew — EDR-aware two-step wait: AppState grows to 8 then the visible layout flips once all PTYs are ready; TLM-06..13 rearrange: TLM-07 short-press-stays-a-click N=5, TLM-08 long-press-on-title-arms N=5, TLM-08b long-press-on-header-BLANK-arms N=5 (the 2026-07-29 hit-area regression), TLM-09 PTY-survives-reorder, TLM-10 drag-reorders, TLM-11 numbering-follows-position, TLM-12 menu-arms, TLM-13 ESC-exits; TLM-14/TLM-15 the Prompt panel's Task selector follows a layout switch and a drag reorder — see the Task-selector row in § 2.7) + `test/unittest/task-layout-utils.test.mts` (TLM-U-01..41) + `test/unittest/task-reorder.test.mts` (TRO-U-01..32 — insert-shift list math, slot hit-testing, FLIP shift offsets incl. the custom-layout scale term, ghost clamping) |
...
| **Slice 3/6 of the GDS split** — `reentry` group = subdir-scope watch + re-entry-content body refresh + re-entry-latency trend + draft-preserved-during-external-refresh (GDS-15, 17, 18, 20) + read-path stat-revalidation surfaces a watcher-missed external edit (GDS-47) + warm-reopen restore-vs-reveal (2026-07-18 staleness bundle: GDS-52 = content changed while closed → the reopen+click must APPLY a content-changed reveal targeting the first change (~line 600 of the 1200-line `deepChangeTargetFile` fixture) with no manual refresh — gated on `getLastRestoreDecision().revealTargetLine ∈ [590,610]` (deterministic from content; pixel scroll offsets are enrichment-only because Monaco measures the diff layout asynchronously after remount); GDS-53 = unchanged content → the reopen+click takes a RESTORE decision (`action ∈ {restore-scroll, restore-anchor}`, `trigger !== 'timeout'`) — decision-record contract only; the pixel-level restore is owned by the SVR suite because `hideUnchangedRegions` makes raw offsets a function of Monaco's async collapse layout; both chained after GDS-17 to reuse its cold-diff cost). Absorbs two of the heaviest singles (GDS-17/20 ~34.6s each) so no slice clusters the expensive cases; its trace markers re-assert snapshot-capture / file-load / model-sync under reentry-owned IDs. Paired unit coverage for the restore-vs-reveal decision table: `git-diff-view-memory.test.mts` (DRD-01..10). **2026-07-27 (BUG-0004): GDS-52's fixture had v1 and v2 both editing line 600, so its `[590,610]` band passed whether the reveal read the CURRENT diff or the previous one — the assertion was structurally incapable of failing on a stale read, which is why it stayed green through the 2026-07-26 bundle. v2's edit moved to line 200 and the case now also fails explicitly on the stale-v1 band.** GDS-54/55 added for the transition GDS-52 never crosses: view while UNTRACKED (empty base ⇒ the whole file is one change at line 1 AND Monaco records ZERO unchanged regions) → stage → edit a LATE line → reopen and click, waiting out the body prefetch so the click is a MEMORY HIT (a cache miss routes through `loading` and takes the safe deferral path, which is the second reason GDS-52 never reproduced it). GDS-54 gates the reveal line (`revealTargetLine===1` is the bundle's signature), GDS-55 gates the collapse state; both aggregate 5 internal trials. | `run-git-diff-reentry` (GDS-15, GDS-17, GDS-18, GDS-20, GDS-47, GDS-52, GDS-53, GDS-54, GDS-55 + GDS-17b/20b trace markers) |
...
| **Mutation-timing matrix (BUG-0004)** — the working tree changes at an ARBITRARY moment relative to the Git Diff lifecycle, which is the normal condition under Agent Coding First. Eight phases: P1 never-opened, P2 closed round-trip (run BOTH low→high and high→low, because a stale read only fails one direction depending on which line is smaller), P3 during-load, P4 during-select (the same few frames in which the 2026-07-26 bundle's decision was taken 47 ms after the body was bound), P5 while-viewing, P6 after-scroll-away, P7 during-close, P8 rapid burst. Deterministic phases (P1/P2/P7/P8) pin the EXACT reveal line against a 1200-line fixture whose edit MOVES between versions — the property GDS-52 lacked, where v1 and v2 both edited line 600 so a stale and a fresh read produced the same number and the assertion could not tell them apart. Racy phases (P3–P6) are invariant-gated instead of line-gated (pinning a line there would over-specify a legitimate race): bound models current + `diffCurrent` + not stranded on line 1 + collapsed + live model count bounded. P8 doubles as the model-leak gate that content-identity URIs make necessary. Split THREE ways BY PHASE so a red box names the moment that broke; each group owns its own app session and temp-dir fixture repo. All cases aggregate 3 internal trials (boolean-correctness, and the defect is a race between Monaco's 200 ms diff debouncer and a rAF tick). | `run-git-diff-mutation-timing-closed` (MT-00, MT-01, MT-02-forward/backward) + `run-git-diff-mutation-timing-load-reveal` (MT-03, MT-04) + `run-git-diff-mutation-timing-viewing` (MT-05..MT-08) + `test/unittest/git-diff-model-identity.test.mts` (MID-01..10, MID-H1..H5 — identity purity + the same-length/same-head/same-tail collision case) + `test/unittest/git-diff-view-memory.test.mts` (WRG-01..07 warm-ready gate decision table) |
...
| Bug fix: SIGTERM/SIGINT could never terminate the app — Chromium converts the signal into the quit flow, which landed in `before-quit` → `confirmQuit()` → `-[NSAlert runModal]` blocked forever (OS shutdown/logout, `pkill -x`, and the orchestrator's `kill_app()` were all ineffective; with ONWARD_PERF_TRACE=1 a flush-only `process.once` handler swallowed the signal first). Fix = main-process SIGTERM/SIGINT handlers route to the bounded no-confirm graceful quit (`requestQuitForDebug`, 12s prod hard-exit floor; pre-ready → `app.exit(0)`); perf-trace flush handler re-raises in workers only; `kill_app()` gained a SIGKILL escalation after an 8s grace. POSIX-only suite: 5× SIGTERM + 1× SIGINT from cold start, every trial must exit code 0 within the design-derived 20s budget. | `run-signal-quit` (SQ-01..06) + `test/unittest/perf-trace-signal-policy.test.mts` (PTS-U-01/02, the main-vs-worker re-raise ownership split) |
...
| Task git badge ahead/behind + background auto-fetch: the branch's upstream ahead (`↑N`) / behind (`↓M`) counts are parsed for free from the already-requested `# branch.ab` porcelain header and carried through the mirror snapshot → badge; a clean tree that is ahead shows a distinct green dot (`#4ADE80`, vs clean emerald), behind rides the `↓` arrow only. Behind is refreshed by a hardened background `git fetch` (`git-autofetch-manager.ts` + pure `git-autofetch-scheduler.ts`: 10-min default period, dedup per repo root, app-hidden pause, per-repo exponential backoff 10 min→1 h, kill switch `ONWARD_DISABLE_GIT_AUTOFETCH`). Fetch is hardened to never prompt/hang (`GIT_TERMINAL_PROMPT=0`, SSH `BatchMode`, 20 s timeout) and never blocks the local status path; on success it revalidates the repo so `# branch.ab` re-reads. Traced at `main:git-autofetch.{scheduled,started,succeeded,failed,backoff,skipped-hidden,triggered-recompute,priority-retry,streak-halved}`. **BUG-0005 fix (2026-08-04)**: a repo whose fetch always timed out was pinned at the 1 h backoff ceiling forever with NO user-reachable escape, so `↓behind` went 98.8 h without a single update while the badge kept presenting it as current. Four changes — (a) the 20 s timeout branch now classifies stderr instead of discarding it (`classified`/`exitCode`/`killedByTimeout`/`stderrTail`/`remoteScheme` on the failure event; the pure tables live in `git-fetch-failure-classify.ts`), (b) focusing a Task grants its backed-off repo ONE backoff-bypassing fetch (≥60 s since last attempt, 5 min per-repo cooldown, streak NOT reset), (c) a hidden→visible edge halves every failure streak, (d) **REWORKED 2026-08-07 after peer research across 7 products** — the count itself is left completely alone (no `↓?`, no muting: no surveyed product marks staleness on the ahead/behind count). Instead the badge becomes the sync ACTION (click = `git.fetchNow`, the app's first manual fetch affordance ever; right-click = copy branch name, which vacated the double-click slot), and an always-present tooltip line carries freshness — `Last synced just now / N min / N h / <date past 24 h>` / `Not synced yet`, the GitHub-Desktop + GitLens treatment. The ONLY failure detail surfaced is `Remote unreachable` (classified `network`); auth / no-remote / bare-timeout stay in the perf trace because they are not actionable from a tooltip. See `docs/html/git-sync-staleness-ux-options.html`. Also fixed alongside: recompute/status durations were measured on the wall clock, so a sample spanning a system sleep (~925 s observed) drove the reconcile backoff to its 60 s cap after every wake — now sanity-checked by `sanitizeMeasuredDurationMs` and traced as `worker:git-state-mirror.duration-suspect`. | `run-git-autofetch-ahead-behind` (AB-01 up-to-date, AB-02 ahead-only, AB-03 behind-only, AB-04 diverged, AB-05 no-upstream, AB-06 background-fetch-flips-behind, **AB-07 fast-failure-carries-evidence**, **AB-08 timeout-failure-carries-evidence** (uses an `ext::sleep` origin so the fetch is genuinely killed by the 20 s ceiling — this case costs the full ceiling by design, hence the 240 s watchdog), **AB-09 success-stamps-last-fetch-ok**, **AB-10 failure-marks-attempt-without-success**, **AB-11 user-fetch-ipc-succeeds** (the production `git.fetchNow` the badge click uses, distinct from AB-06's autotest-only hook), **AB-12 no-remote-does-not-claim-unreachable** / **AB-13 connection-refused-reports-unreachable** (pins the surface-only-network mapping in BOTH directions) — a bare-remote + clones fixture; snapshots asserted via `getMirror`) + `test/unittest/git-porcelain-parse.test.mts` (`# branch.ab` parse incl. malformed/no-upstream) + `test/unittest/git-sync-display.test.mts` (pure dot-colour + arrow decision table **+ the four-state freshness table (`no-upstream` / `never-attempted` / `never-succeeded` / `synced`), the relative→absolute date boundary, the surface-only-unreachable rule incl. a key-shape assertion that stops an error string leaking back in, and the age bucketing that picks the tooltip's i18n key**) + `test/unittest/git-autofetch-scheduler.test.mts` (WHEN-to-fetch: dedup/prune, hidden-pause, kill switch, interval gating, backoff+reset **+ priority-retry grant/refusal matrix incl. cooldown and hidden-pause precedence, streak halving idempotence, overdueSnapshot finiteness**) + `test/unittest/git-fetch-failure-classify.test.mts` (**stderr classification, userinfo/token redaction with tail preservation, remote transport classification incl. scp-like**) + `test/unittest/git-reconcile-scheduler.test.mts` (**duration sanity: smaller-clock preference, ceiling boundary, rejected sample → base cadence not the cap, EDR-slow host still stretches**) + `test/unittest/terminal-grid-git-status-identity.test.mts` (ahead/behind surfaced through the display state + delta merge) |
...
| HTML source editing plus sandboxed DOM iframe preview with custom-protocol path isolation, local file assets, HTTP script access, host context-menu/dialog z-order coverage (5/5 repeated trials), splitter drag, in-document search, HTML Preview zoom, browser-style navigation (back / forward / hard-reload-ignoring-cache / home-to-opened-file with disabled states), Cmd/Ctrl+R hard reload of the current page (in place, no home jump), save-remount history reset, blocked out-of-root navigation, scroll-preserving fresh reload on save without cross-document transplant, idempotent rapid reload, subframe-navigation isolation from top-level nav state, persisted HTML scroll memory (`FileViewMemory.htmlScrollX/Y`: survives file-switch-and-back, ESC close→reopen, appState round-trip), remembered HTML Preview zoom re-applied to the live document across ESC close→reopen (UI factor + iframe controller + `documentElement.style.zoom` all locked) and across in-place reload / link navigation, in-page TOC anchor jumps (bridge-side programmatic scroll: first click, repeat-click on the same entry 5×, no stuck loading overlay, correct landing under 1.5× CSS zoom, pure-scroll semantics — no Back/Forward/Home history mutation, user click wins over warm-reopen scroll restore), same-URL link clicks reloading in place 5× with a deduped single history entry, preview-link viewer dispatch (a link to a local non-HTML project file opens the matching Project Editor viewer instead of dead-ending the iframe: HTML link → Markdown viewer, Markdown preview link → source editor, one-click "Back to {file}" return bar for both source kinds, out-of-root Markdown link blocked with an error status), and the full link-matrix acceptance (external http(s) links — plain click, `target="_blank"`, `window.open` — hand off to the Open Browser webview with editor soft-close + auto-return round trip; absolute `file://` links resolve against the root into iframe/viewer routes; `mailto:` goes to the OS handler via the autotest-stubbed confirm guard; `data:` stays blocked) | `run-project-editor-html-preview` (PHTML-00..51 + PHTML-04c..04d + PHTML-36b) + `test/unittest/html-preview-protocol.test.mts` (HPP-U-01..14, incl. the raw-href in-page-anchor classifier + the `classifyHtmlPreviewLink` route table with file:// and mailto/tel routes) + `test/unittest/preview-link-dispatch.test.mts` (PLD-U-01..07, the Markdown preview href router) + `test/unittest/project-editor-html-file.test.mts` (PEHTML-U-01..26, incl. hash-insensitive Home enablement) + `test/unittest/app-state-file-states-validation.test.mts` (AFVM-U-*, persistence whitelist incl. the previously-dropped PDF/EPUB fields) |
...
| Prompt sender action buttons, send/execute | `run-prompt-sender` (PS-01, PS-03..10) |
| **Task selector mirrors the Task layout (2026-07-29).** The Prompt panel's Task selection box is a scale model of the Task grid, not a fixed 2-column list: preset layouts use the grid's own track counts (1→1×1, 2→2×1, 4→2×2, 6→3×2, 8→4×2) and a custom layout reproduces each Task's stored rectangle inside the 4×2 atomic mesh, spans included. Rows are fixed-height so a row-spanning custom cell reads as double height here too. Slot→Task binding is by index, which is what makes drag-to-rearrange propagate with no reorder-specific code — both surfaces re-derive placement from the same Task array. A slot too narrow for a name (≈8 Tasks in a 320px panel) swaps the name for the Task's position number via a container query on the card, full name in the tooltip. The selection swatch beside the action buttons uses the same tracks and rectangles. | `run-prompt-sender` (PS-02 mirrors the live grid's computed tracks, PS-34 per-card rectangles + DOM order vs. grid cells, PS-35 indicator swatch, PS-36/PS-36b narrow-slot degradation both ways) + `run-task-layout` (TLM-14 selector follows a layout switch, TLM-15 selector follows a drag reorder) + `test/unittest/task-selector-geometry.test.mts` (TSG-U-01..17 — TSG-U-01/02 parse `TerminalGrid.css` and fail if the preset track table drifts from the real grid CSS) |
