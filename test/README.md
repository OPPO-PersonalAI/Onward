...
| Cmd+P arrow-key navigation — per-press latency against a one-frame budget (16 ms, agreed 2026-08-07; 3 trials, pass if any median is within budget), selected row never leaves the scroll viewport, the press at the page boundary is not swallowed (prefetch one screen ahead + deferred step), hover cannot hijack a keyboard-driven selection while a real pointer move still restores it, and the keyboard hint bar / position counter are rendered | `run-quick-open-key-latency` (QOKL-01..10) + `test/unittest/list-keyboard-nav.test.mts` (LKN-U-1..4: step/clamp/defer decision table, prefetch window, minimal-scroll follow math incl. idempotence and pre-layout no-op, hover arbitration) |
