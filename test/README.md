...
| Bug fix (2026-07-13): all terminals white 1–3 s after Space switch-back — occlusion keep-alive contract: `document.hidden` never disposes WebGL (5-trial roundtrip aggregate, own-accessor `document.hidden` shadowing drives the real hidden branch), and the host-surface restore repaints within `SURFACE_RESTORE_BUDGET_MS = 200` (3 trials, ≥1 within budget; user-approved 2026-07-13). Pure restore decision table (`refresh-only` / `recreate-webgl` / `defer-*` / `keep-alive`) is unit-pinned. Phase 2 (2026-07-14): the Electron 39 ANGLE-Metal GPU-process crash on Space switches never delivers `webglcontextlost` — main watches `child-process-gone`(GPU) and broadcasts; the session manager force-recreates all visible WebGL (TFA-21, real IPC roundtrip via the autotest-gated simulate hook; wiring-only change, no pure-logic surface for the unit layer) | `run-terminal-focus-activation` (TFA-19, TFA-20, TFA-21) + `test/unittest/terminal-renderer-surface-policy.test.mts` (TRSP-U-01..09) |
...
| PDF reader / EPUB reader inside editor (incl. iframe → host keyboard forwarding for BOTH readers — pdf.js `onward:pdf:hostKey` postMessage AND epub.js rendition-keydown relay so ESC inside the EPUB content iframe closes the editor (`epub-escape-forwarded-from-content`, N=3) — plus ESC + shortcut reopen keeps iframe / reader mounted across N=5 close-retain cycles) | `run-pdf-epub-preview` (`pdf-reader-*`, `epub-*`) + `test/unittest/reader-host-key-forwarding.test.mts` (RHK-U-01..04, shared Escape/Cmd+P allowlist) |
...
| Telemetry session start, properties, heartbeat; PostHog upload-client not-configured fallback (`disabled` sentinel key); outbox acknowledgement semantics — record on disk = not confirmed uploaded, backlog remediated via historical_migration + current-day Tier-2 live lane to a local mock ingest server, cleared only after the flush ack (launch B), retained without ack (launch C); 2026-07 metric redesign P1+P2 — feature/first-use once-per-install derivation, extended daily-stats domains (stability/update/codeAgent), generic feature/use counter map with allowlist clamping | `run-telemetry` `.sh`+`.ps1` (TEL-01..11 + TEL-14/TEL-14b/TEL-15/TEL-17/TEL-17b/TEL-17c launch A, TEL-12a..c + TEL-16 ack path, TEL-13a..b nack path); unit `telemetry-outbox.test.mts` (TOB-01..08), `telemetry-posthog-backfill-mapping.test.mts` (TBF-01..06), `telemetry-event-names.test.mts` (TEN-01..07 allowlist/first-use/tiering/feature-use), `telemetry-daily-stats.test.mts` (TDS-01..04 extended counters + featureUse map + schema merge) |
