...
| Bug fix (2026-07-13): all terminals white 1–3 s after Space switch-back — occlusion keep-alive contract: `document.hidden` never disposes WebGL (5-trial roundtrip aggregate, own-accessor `document.hidden` shadowing drives the real hidden branch), and the host-surface restore repaints within `SURFACE_RESTORE_BUDGET_MS = 200` (3 trials, ≥1 within budget; user-approved 2026-07-13). Pure restore decision table (`refresh-only` / `recreate-webgl` / `defer-*` / `keep-alive`) is unit-pinned | `run-terminal-focus-activation` (TFA-19, TFA-20) + `test/unittest/terminal-renderer-surface-policy.test.mts` (TRSP-U-01..09) |
...
| Rich-file cold Diff / History → Editor → source round trips | `run-subpage-navigation-html` (`SNJ-HTML-{DIFF,HISTORY}-COLD-5X`) + `run-subpage-navigation-pdf` (`SNJ-PDF-{DIFF,HISTORY}-COLD-5X`) + `run-subpage-navigation-epub` (`SNJ-EPUB-{DIFF,HISTORY}-COLD-5X`); every trial uses a different file that Editor has not mounted before |
| Rich-file warm state restoration across Diff / History round trips | `run-subpage-navigation-html` (`SNJ-HTML-*-WARM-5X`: working-tree HTML, relative CSS/JS, Preview, scrollY, zoom) + `run-subpage-navigation-pdf` (`SNJ-PDF-*-WARM-5X`: working-tree v2 text, page 2, zoom, internal scroll) + `run-subpage-navigation-epub` (`SNJ-EPUB-*-WARM-5X`: working-tree text, unique reader session, latest settled attempt, chapter 2 CFI, 110% label + computed font size, internal scroll, stale-callback stability); paired reader-state coverage: `project-editor-pdf-reader-state.test.mts`, `project-editor-epub-reader-state.test.mts` |
...
| HTML source editing plus WebContents preview with local file assets, HTTP script access, splitter drag, WebContents search, HTML Preview zoom, browser-style navigation (back / forward / hard-reload-ignoring-cache / home-to-opened-file with disabled states), Cmd/Ctrl+R hard reload of the current page (in place, no home jump), save-remount history reset, blocked out-of-root navigation, scroll-preserving fresh reload on save without cross-document transplant, idempotent rapid reload, subframe-navigation isolation from top-level nav state, and persisted HTML scroll memory (`FileViewMemory.htmlScrollX/Y`: survives file-switch-and-back, ESC close→reopen, appState round-trip) | `run-project-editor-html-preview` (PHTML-00..31) + `test/unittest/app-state-file-states-validation.test.mts` (AFVM-U-*, persistence whitelist incl. the previously-dropped PDF/EPUB fields) |
